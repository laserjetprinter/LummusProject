...
while True:
    ...
    if raw_adc > 32767:
	    raw_adc -= 65535
    pressure1Unrounded = (raw_adc/65535)*15
    pressure1 = round(pressure1Unrounded,3)
    print("Pressure : %d" %pressure1)
    ...
    if raw_adc > 32767:
	    raw_adc -= 65535
    tempUnrounded = (raw_adc/65535) * 1092
    temp = round(tempUnrounded,3)
    ...
    if raw_adc > 32767:
        raw_adc -= 65535
    pressure2Unrounded = (raw_adc / 65535) * 15
    pressure2 = round(pressure2Unrounded, 3)
    print("Pressure : %d" % pressure2)
    ...
    App = 1         #Area of gap between plunger and pipe (m^2)
    AMetric = 1     #Cross sectional area of pipe (m^2)
    cMetric = 1     #Maximum seed cotton velocity (m/s)
    vbbMetric = 1   #air velocity through the blowbox (m/s)
    ksMetric = 1    #Friction loss factor for seed cotton through blowbox
    vinMetric = 1   #measured air velocity at blowbox inlet (m/s)
    
    RMetric = 287.058       #Gas constant (J/kg-K)
    Patm = 101325           #Atmospheric pressure in Pascals
    pstarMetric = 101325    #Density of higher-pressure air (kg/m^3) ... assuming equal to Patm
    ...
    vout = math.sqrt((2)*(Patm - P1Metric) / (pinMetric))
    ...
    #Constants found before testing. Default value set to 1 for debugging
    di = 1          #Inner diameter (ft)
    L = 1           #Equivalent length (ft)
    V1 = 1          #Velocity of air (ft/s)
    AEng = 1        #Area of cross section of inner pipe (ft^2)
    KEng = 1        #Friction Factor Multiplier (unitless)
    u1 = 1          #Viscosity of air (lb/ft-s)
