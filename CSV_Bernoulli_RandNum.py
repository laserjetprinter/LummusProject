...
while True:
    ...
    pressure1 = random.uniform(14.0, 15.0)
    pressure2 = random.uniform(14.0, 15.0)
    ...
    vout = math.sqrt((2)*(abs(P1Metric - Patm)) / (pinMetric))
    ...
    part2 = ((abs(P2Metric -P1Metric)) / (vinMetric + (Q2/AMetric))) - ((ksMetric/2) * (pinMetric * vinMetric)) + ((ksMetric/2) * (Q2/AMetric) * pstarMetric)
    mrate = part1 * part2
    ...
    P1Eng = pressure1       #Pressure sensor reading of air before mixture in PSI NOT lbs per sqft
    P2Eng = pressure2       #Pressure sensor reading of cottain air mixture in PSI NOT lbs per sqft
    ...
    numerator = AEng*(abs(P2Eng-P1Eng))*(144*32.2)
    mratebat = numerator/(0.8*V1)
